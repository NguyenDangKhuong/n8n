  ...
  if (transportType === 'sse') {
    ...
    app.post('/mcp', async (req, res) => {
      ...
      if (method === 'tools/list' || method === 'list_tools') {
        ...
        // Handle tool calls directly using the same logic as the MCP server
        const { name, arguments: args } = req.body.params || {};
        console.error(`Hybrid handler: tools/call → ${name}`);
        ...
        try {
          ...
          const limited = limitResponse(result, RESPONSE_LIMITS.MAX_RESPONSE_SIZE);
          const responseData = limited.truncated
            ? { ...limited.data, _meta: { truncated: true, originalSize: limited.originalSize, pagination: limited.pagination } }
            : limited.data;
          ...
          console.error(`Hybrid tool error (${name}):`, error);
